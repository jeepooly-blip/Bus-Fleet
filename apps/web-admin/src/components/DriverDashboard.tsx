  ...
  return (
    <div className="space-y-6 md:y-8">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4">
        <div>
          <p className="text-[10px] font-mono text-primary/40 uppercase tracking-widest mb-1">Shift in progress</p>
          <h2 className="text-2xl md:text-3xl font-serif italic">Welcome, {driver?.full_name || 'Driver'}</h2>
        </div>
        <div className="text-right">
          <p className="text-[10px] md:text-xs font-bold uppercase tracking-tighter text-green-600 flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
        ...
        <div className="lg:col-span-2 space-y-6 md:space-y-8">
          <div className="card p-6 md:p-8 relative overflow-hidden">
            ...
            <div className="flex items-center gap-3 mb-8">
              ...
              <h3 className="text-sm md:text-lg font-bold uppercase tracking-widest">Active Route</h3>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12">
              <div className="space-y-6">
                <div>
                  <label className="text-[10px] font-mono text-primary/40 uppercase tracking-widest block mb-2">Route Name</label>
                  <p className="text-xl md:text-2xl font-serif italic">{activeTrip.route.name}</p>
                </div>
                <div className="flex items-center gap-4 md:gap-8">
                  <div>
                    <label className="text-[10px] font-mono text-primary/40 uppercase tracking-widest block mb-1">Origin</label>
                    <p className="text-xs md:text-sm font-bold">{activeTrip.route.origin}</p>
                  </div>
                  <div className="w-8 h-px bg-primary/10"></div>
                  <div>
                    <label className="text-[10px] font-mono text-primary/40 uppercase tracking-widest block mb-1">Destination</label>
                    <p className="text-xs md:text-sm font-bold">{activeTrip.route.destination}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-6">
                <div className="flex items-center gap-4 p-4 bg-secondary rounded-2xl">
                  ...
                  <div>
                    <p className="text-[10px] font-mono text-primary/40 uppercase tracking-widest">Bus Plate</p>
                    <p className="text-base md:text-lg font-bold tracking-tighter">{activeTrip.bus.plate_number}</p>
                  </div>
                </div>
                ...
                <div className="pt-4 border-t border-primary/5">
                  <AnimatePresence mode="wait">
                    {!showCompleteConfirm ? (
                      ...
                      >
                        <p className="text-[10px] font-bold text-green-800 text-center uppercase tracking-widest">Confirm Completion?</p>
          ...
          <div className="card p-6 md:p-8">
            <div className="flex items-center justify-between mb-8">
              <div className="flex items-center gap-3">
                ...
                <h3 className="text-sm md:text-lg font-bold uppercase tracking-widest">Route Compliance</h3>
              </div>
              <span className="text-[10px] font-mono text-primary/40 uppercase tracking-widest hidden sm:inline">Verification Tool</span>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-4 mb-6">
              <div className="relative flex-1">
                ...
                <input 
                  ...
                  className="w-full pl-12 pr-4 py-4 bg-secondary border-none rounded-xl text-sm focus:ring-2 focus:ring-primary/10 outline-none"
                />
              </div>
              <button 
        ...
        <div className="space-y-8">
          ...
          <div className={`rounded-3xl p-8 shadow-xl transition-all duration-500 relative overflow-hidden ${activeTrip.bus.panic_status ? 'bg-red-600 text-white animate-pulse' : 'bg-primary text-white'}`}>
            ...
            <div className="text-center py-4 md:py-8 relative z-10">
              <button 
                onClick={handlePanic}
                disabled={panicLoading}
                className={`w-24 h-24 md:w-32 md:h-32 rounded-full border-4 border-white/20 flex flex-col items-center justify-center gap-2 transition-all active:scale-95 mx-auto ${activeTrip.bus.panic_status ? 'bg-white text-red-600 shadow-2xl' : 'bg-red-600 hover:bg-red-700'}`}
              >
                <AlertTriangle size={activeTrip.bus.panic_status ? 24 : 32} className={activeTrip.bus.panic_status ? 'animate-bounce' : ''} />
                <span className="text-[10px] font-bold uppercase tracking-widest">
                  {activeTrip.bus.panic_status ? 'CANCEL' : 'PANIC'}
                </span>
              </button>
              <p className="mt-6 text-[10px] md:text-xs text-white/60 leading-relaxed max-w-[200px] mx-auto">
                {activeTrip.bus.panic_status 
                  ? "EMERGENCY SIGNAL SENT. Dispatch has been notified."
                  : "Press in case of emergency. This will alert dispatch immediately."
                }
              </p>
            </div>
          </div>
          ...
          <div className="card p-8">
            <h3 className="text-[10px] font-mono text-primary/40 uppercase tracking-widest mb-6">Vehicle Status</h3>
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                ...
                <span className="text-[10px] font-mono text-primary/40">OPTIMAL</span>
              </div>
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="w-2 h-2 bg-green-500 rounded-full"></div>
                ...
                <span className="text-[10px] font-mono text-primary/40">STRONG</span>
              </div>
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                ...
                <span className="text-[10px] font-mono text-primary/40">65%</span>
              </div>
            </div>
          </div>
        </div>
