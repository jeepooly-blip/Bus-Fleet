...
export const LiveMap: React.FC = () => {
  ...
  return (
    <div className="relative w-full h-[400px] md:h-[600px] rounded-2xl md:rounded-3xl overflow-hidden border border-primary/10 shadow-lg" onClick={() => setSelectedBusId(null)}>
      {error ? (
        <div className="absolute inset-0 flex items-center justify-center bg-secondary p-8 text-center">
          <div className="max-w-md">
            <p className="text-red-600 font-bold mb-2 uppercase tracking-widest text-[10px]">Configuration Error</p>
            <p className="text-xs md:text-sm text-primary/60">{error}</p>
          </div>
        </div>
      ) : (
      ...
      <div className="absolute top-4 left-4 right-4 sm:right-auto bg-white/90 backdrop-blur-md p-0 rounded-xl shadow-2xl border border-primary/10 z-10 sm:w-56 overflow-hidden">
        <div className="bg-primary text-white px-4 py-2 flex justify-between items-center">
          <div>
            <h3 className="text-[8px] font-mono uppercase tracking-[0.2em] opacity-80">System Monitor</h3>
            <p className="text-[10px] font-bold italic font-serif">Amman Live Fleet</p>
          </div>
          <div className="w-1.5 h-1.5 bg-green-400 rounded-full animate-pulse" />
        </div>
        
        <div className="p-1">
          <div className="grid grid-cols-4 px-3 py-1.5 border-b border-primary/5">
            <span className="col-span-2 text-[8px] font-serif italic uppercase opacity-40">Status</span>
            <span className="text-[8px] font-serif italic uppercase opacity-40 text-right">Count</span>
            <span className="text-[8px] font-serif italic uppercase opacity-40 text-right">Filter</span>
          </div>

          <button 
            onClick={(e) => { e.stopPropagation(); setStatusFilter(statusFilter === 'active' ? null : 'active'); }}
            className={`grid grid-cols-4 items-center w-full px-3 py-2 transition-all duration-200 group border-b border-primary/5 ${statusFilter === 'active' ? 'bg-primary text-white' : 'hover:bg-primary/5'}`}
          >
            <div className="col-span-2 flex items-center gap-2">
              <div className={`w-1.5 h-1.5 rounded-full ${statusFilter === 'active' ? 'bg-green-400' : 'bg-green-500/60'}`} />
              <span className="text-[9px] font-mono uppercase tracking-wider">Active</span>
            </div>
            <span className="text-[9px] font-mono text-right">{counts.active}</span>
            <div className="flex justify-end">
              <div className={`w-2.5 h-2.5 border border-current rounded-sm flex items-center justify-center ${statusFilter === 'active' ? 'bg-white' : ''}`}>
                {statusFilter === 'active' && <div className="w-1.5 h-1.5 bg-primary" />}
              </div>
            </div>
          </button>
          ...
          <button 
            ...
            className={`grid grid-cols-4 items-center w-full px-3 py-2 transition-all duration-200 group border-b border-primary/5 ${statusFilter === 'maintenance' ? 'bg-primary text-white' : 'hover:bg-primary/5'}`}
          >
            <div className="col-span-2 flex items-center gap-2">
              <div className={`w-1.5 h-1.5 rounded-full ${statusFilter === 'maintenance' ? 'bg-yellow-400' : 'bg-yellow-500/60'}`} />
              <span className="text-[9px] font-mono uppercase tracking-wider">Service</span>
            </div>
            <span className="text-[9px] font-mono text-right">{counts.maintenance}</span>
            <div className="flex justify-end">
              <div className={`w-2.5 h-2.5 border border-current rounded-sm flex items-center justify-center ${statusFilter === 'maintenance' ? 'bg-white' : ''}`}>
                {statusFilter === 'maintenance' && <div className="w-1.5 h-1.5 bg-primary" />}
              </div>
            </div>
          </button>
          ...
          <button 
            ...
            className={`grid grid-cols-4 items-center w-full px-3 py-2 transition-all duration-200 group ${statusFilter === 'inactive' ? 'bg-primary text-white' : 'hover:bg-primary/5'}`}
          >
            <div className="col-span-2 flex items-center gap-2">
              <div className={`w-1.5 h-1.5 rounded-full ${statusFilter === 'inactive' ? 'bg-red-400' : 'bg-red-500/60'}`} />
              <span className="text-[9px] font-mono uppercase tracking-wider">Offline</span>
            </div>
            <span className="text-[9px] font-mono text-right">{counts.inactive}</span>
            <div className="flex justify-end">
              <div className={`w-2.5 h-2.5 border border-current rounded-sm flex items-center justify-center ${statusFilter === 'inactive' ? 'bg-white' : ''}`}>
                {statusFilter === 'inactive' && <div className="w-1.5 h-1.5 bg-primary" />}
              </div>
            </div>
          </button>
        </div>
        ...
        {(selectedBusId || statusFilter) && (
          <button 
            ...
            className="w-full py-2 bg-primary text-white text-[8px] font-mono uppercase tracking-[0.2em] hover:bg-red-600 transition-colors border-t border-white/10"
          >
            Reset System View
          </button>
        )}
